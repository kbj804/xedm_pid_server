...
class BaseMixin:
    ...
    # 인스턴스 메소드는 객체가 생성된 후 실행 / 클래스 메소드는 객체 생성이 되지않은 상태에서 실행
    @classmethod
    def get(cls, session: Session = None, **kwargs):
        ...
        :return:
        """
        # 세션을 새로 만들고 연결 한 후에 동작이 끝나면 세션 종료.. 트래픽 고려 더 해봐야 함.
...
class Train(Base, BaseMixin):
    __tablename__= "train"
    reg_count = Column(Integer, nullable=True, default=0)
    for i in range(1,11):
        locals()[f'column{str(i)}'] = Column(Integer, nullable=True, default=0)
    y = Column(Integer, nullable=True, default=0)
    page = Column(Integer, nullable=True, default=0)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
